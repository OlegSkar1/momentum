...
function loadMusic(musicIndex) {
  ...
  mainAudio.src = playList[musicIndex].src;
...
for (let i = 0; i < playItems.length; i++) {
  playItems[i].setAttribute('li-index', i);
}

function playingSong() {
  for (let i = 0; i < playItems.length; i++) {
    if (playItems[i].classList.contains('play-item-active')) {
      playItems[i].classList.remove('play-item-active');
    }
    if (playItems[i].getAttribute('li-index') == playNum) {
      playItems[i].classList.add('play-item-active');
    }
  }
}

function clicked(event) {
  let getLiElement = event.target.getAttribute('li-index');
  playNum = getLiElement;
  loadMusic(playNum);
  playMusic();
  playingSong();
}
ul.addEventListener('click', (event) => clicked(event));
